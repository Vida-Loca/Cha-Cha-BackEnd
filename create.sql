...
create table products (product_id integer not null auto_increment, name varchar(255) not null, price double precision, product_category_id integer, primary key (product_id)) engine=InnoDB
create table roles (role_id integer not null auto_increment, name varchar(255) not null, primary key (role_id)) engine=InnoDB
create table user_cards (user_card_id integer not null auto_increment, event_id integer, product_id integer, user_id bigint, primary key (user_card_id)) engine=InnoDB
...
alter table user_cards add constraint FK9k4fhdvkcuq1g93eu8mnxl1sl foreign key (event_id) references events (event_id)
alter table user_cards add constraint FKensu68nl6p202jl2o7daygkpr foreign key (product_id) references products (product_id)
